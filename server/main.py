...
def _hydrate_from_packet(parsed: Any, data: Dict[str, Any]) -> None:
    ...
    if isinstance(parsed, packets.PacketCarStatusData_V1):
        ...
        if car is not None:
            ...
            data["tyresDamage"] = getattr(car, "tyresDamage", None)
            ...
            data["frontLeftWingDamage"] = getattr(car, "frontLeftWingDamage", None)
            data["frontRightWingDamage"] = getattr(car, "frontRightWingDamage", None)
            data["rearWingDamage"] = getattr(car, "rearWingDamage", None)
            data["engineDamage"] = getattr(car, "engineDamage", None)
            data["gearBoxDamage"] = getattr(car, "gearBoxDamage", None)
            data["drsFault"] = getattr(car, "drsFault", None)
            data["vehicleFiaFlags"] = getattr(car, "vehicleFiaFlags", None)
        return
...
def _to_json_list(value: Any) -> Optional[List[float]]:
    if value is None:
        return None
...
def _telemetry_context() -> str:
    ...
    tyres_wear = _to_json_list(snapshot.extra.get("tyresWear"))
    tyres_damage = _to_json_list(snapshot.extra.get("tyresDamage"))
    tyre_wear_text = None
    tyre_damage_text = None
    ...
    if isinstance(tyres_damage, (list, tuple)) and len(tyres_damage) >= 4:
        tyre_damage_text = f"FL={tyres_damage[0]}, FR={tyres_damage[1]}, RL={tyres_damage[2]}, RR={tyres_damage[3]}"
    ...
    parts = [
        ...
        f"tyre_damage={tyre_damage_text}",
        ...
        f"ers_mode={ers_mode}",
        f"front_left_wing_damage={snapshot.extra.get('frontLeftWingDamage')}",
        f"front_right_wing_damage={snapshot.extra.get('frontRightWingDamage')}",
        f"rear_wing_damage={snapshot.extra.get('rearWingDamage')}",
        f"engine_damage={snapshot.extra.get('engineDamage')}",
        f"gearbox_damage={snapshot.extra.get('gearBoxDamage')}",
        f"drs_fault={snapshot.extra.get('drsFault')}",
...
def _chat_response(message: str, history: List[Dict[str, str]]) -> str:
    telemetry_text = _telemetry_context()
    system_prompt = f"""You are an F1 2020 race engineer and strategy advisor.
...
- tyre_damage: per-tyre damage percentages (FL/FR/RL/RR) from car status.
...
def telemetry_status() -> Dict[str, Any]:
    ...
    return {
        "udp_host": UDP_HOST,
        "udp_port": UDP_PORT,
        "parser_status": telemetry_parser.status,
        "last_packet_id": snapshot.raw_packet_id if snapshot else None,
        "last_received_at": snapshot.received_at if snapshot else None,
    }
@app.post("/api/chat", response_model=ChatResponse)
def chat(payload: ChatRequest) -> ChatResponse:
    response = _chat_response(payload.message, payload.history)
    return ChatResponse(response=response)


@app.websocket("/ws/telemetry")
async def telemetry_ws(socket_ws: WebSocket) -> None:
    await socket_ws.accept()
    try:
        while True:
            snapshot = telemetry_store.get()
            if snapshot is None:
                await socket_ws.send_json({"status": "no_data"})
            else:
                gap_info = _compute_gaps(snapshot)
                weather = snapshot.extra.get("weather")
                safety_car = snapshot.extra.get("safetyCarStatus")
                ...
                payload = {
                    ...
                    "lap": snapshot.lap,
                    "total_laps": snapshot.extra.get("totalLaps"),
                    "position": snapshot.position,
                    "tyre_compound": snapshot.tyre_compound,
                    "tyre_wear": _to_json_list(snapshot.extra.get("tyresWear")),
                    "tyre_damage": _to_json_list(snapshot.extra.get("tyresDamage")),
                    "fuel_kg": snapshot.fuel_kg,
                    "fuel_laps": snapshot.extra.get("fuelRemainingLaps"),
                    "weather": weather,
                    "safety_car": safety_car,
                    "speed_kph": snapshot.extra.get("speed"),
                    "drs": snapshot.extra.get("drs"),
                    "ers_energy": snapshot.extra.get("ersStoreEnergy"),
                    "ers_mode": snapshot.extra.get("ersDeployMode"),
                    "gap_ahead_m": gap_info.get("gap_ahead_m"),
                    "gap_ahead_s": gap_info.get("gap_ahead_s"),
                    "gap_behind_m": gap_info.get("gap_behind_m"),
                    "gap_behind_s": gap_info.get("gap_behind_s"),
                    "safety_car_delta": snapshot.extra.get("safetyCarDelta"),
                    "front_left_wing_damage": snapshot.extra.get("frontLeftWingDamage"),
                    "front_right_wing_damage": snapshot.extra.get("frontRightWingDamage"),
                    "rear_wing_damage": snapshot.extra.get("rearWingDamage"),
                    "engine_damage": snapshot.extra.get("engineDamage"),
                    "gearbox_damage": snapshot.extra.get("gearBoxDamage"),
                    "drs_fault": snapshot.extra.get("drsFault"),
                }
                await socket_ws.send_json(payload)
            await _sleep_ws()
    except WebSocketDisconnect:
        return


async def _sleep_ws() -> None:
